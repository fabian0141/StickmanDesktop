...
public class Level1 extends Level{
	...
	public Level1(ObjectList objList, Stances stances, Sound sound, AllWeapons allWeapon, ShaderProgram[] programs) throws Exception {
		super(sound);
	...
	public void input(Controlls controlls) {
		...
		if (!end && (dead || ((Body) objects.get(0)).dead || ((Body) objects.get(1)).dead)) {
			dead = false;
			end = true;
		...
		if ((int) (Math.random() * 30) == 0 && generatedEnemies > 0) {
			objects.add(new Body(new Vector2f(-7.5f - levelPos, -1f + (float) Math.random() / 8), stances, sound, 0,
	...
	private boolean end;
	private int generatedEnemies = 100;
	...
	protected boolean checkArea(int i, float moveX, float moveY) {
		boolean inArea = objects.get(i).getXPos() + moveX < 8 - levelPos;
		inArea &= objects.get(i).getXPos() + moveX > -8 - levelPos;
